...
PLANS = {
    ...
    "S_29": {"price": 29, "credits": 1, "daily": 1, "validity_days": 15, "name_telugu": "ఒక్క Request — 1 profile"},
...
def deduct_credit(user: Dict) -> Dict:
    ...
    if user["credits"] <= 0:
        return {"success": False, "message_telugu": "⚠️ Me credits ayipoyayi! Malli ₹99 తో 10 credits pondandi", "credits": 0}
    user["credits"] -= 1
    # log transaction
    return {"success": True, "credits": user["credits"], "message_telugu": f"✅ Number unlock అయ్యింది! Migilina credits: {user['credits']}"}
...
def add_credits(user: Dict, plan_key: str, payment_verified: bool = True) -> Dict:
    ...
    return {"success": True, "credits": user["credits"], "plan": plan_key, "expiry": user["plan_expiry"], "message_telugu": f"🎉 {plan['credits']} credits add అయ్యాయి! Plan: {plan['name_telugu']}"}

def add_referral_bonus(user: Dict, bonus_credits: int = 2) -> Dict:
    user["credits"] += bonus_credits
    return {"success": True, "credits": user["credits"], "message_telugu": f"🎉 Referral bonus! మీకు {bonus_credits} free credits vachayi"}

def add_admin_gift(user: Dict, gift_credits: int = 10) -> Dict:
    user["credits"] += gift_credits
    user["plan"] = "PREMIUM_299"  # gift as premium
    return {"success": True, "credits": user["credits"], "message_telugu": f"💎 Admin gift! మీకు {gift_credits} credits FREE + Premium! — TSAP Team"}
...
def can_search_id(user: Dict, search_id: str) -> Dict:
    """ID search ఎప్పుడు open — profile చూడొచ్చు, number కి credit కావాలి"""
    return {
        "can_view_profile": True,
        "can_view_number": can_view_number(user["credits"]),
        "is_photo_blur": user["plan"]=="FREE" and not user.get("is_paid", False),  # free ki blur
        "message": "ID search always open — profile చూడొచ్చు, number కి credit కావాలి" if not can_view_number(user["credits"]) else "Full access"
    }
